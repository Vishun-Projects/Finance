...
export async function GET(request: NextRequest) {
  try {
    ...
    // OPTIMIZATION: Execute ALL independent queries in PARALLEL to avoid waterfall
    const [
      transactionStats,
      legacyExpenseStats,
      legacyIncomeStats,
      ...
      legacyCategoryBreakdown
    ] = await Promise.all([
      // 1. Transaction aggregation (Credits/Debits)
      (async () => {
        try {
      ...
      // 5. Upcoming deadlines count
      prisma.deadline.count({ where: { userId, isCompleted: false, dueDate: { gt: new Date() } } }),

      // 6. Recent transactions
      (async () => {
        try {
          // Optimization: Skip existence check if possible, or trust previous check. 
          // For safety in this hybrid codebase, we keep the check but it could be cached.
          // await (prisma as any).$queryRaw`SELECT 1 FROM transactions LIMIT 1`; 
          // We can skip the check here if we assume the first query handles the "table missing" logic globally, 
          // but for robustness in Promise.all we keep Try-Catch.
          return await (prisma as any).transaction.findMany({
            where: { userId, isDeleted: false, transactionDate: { gte: rangeStart, lte: rangeEnd } },
            select: { id: true, description: true, creditAmount: true, debitAmount: true, financialCategory: true, transactionDate: true, store: true, category: { select: { name: true } } },
      ...
      // 7. Category Stats (Financial Category Breakdown)
      (async () => {
        try {
      ...
      // 8. Transaction Totals (for Trends)
      (async () => {
        try {
          return await (prisma as any).transaction.groupBy({
            by: ['transactionDate'],
      ...
      // 9. Legacy Expense Totals (for Trends)
      (prisma as any).expense.groupBy({
        by: ['date'],
        where: { userId, isDeleted: false, date: { gte: rangeStart, lte: rangeEnd } },
        _sum: { amount: true }
      }).catch(() => []),

      // 10. Category Breakdown (Transaction model)
      (async () => {
        try {
          return await (prisma as any).transaction.groupBy({
            by: ['categoryId'],
    ...
    // --- Processing Logic (No new DB queries below this point) ---
    ...
    (expenseTotals as any[]).forEach((t: any) => {
      if (!t.date) return;
      const dateKey = new Date(t.date).toISOString().split('T')[0];
      expenseByDate.set(dateKey, (expenseByDate.get(dateKey) || 0) + Number(t._sum.amount || 0));
    });
    ...
    const categoryMap = new Map<string, string>(categories.map((c: { id: string; name: string }) => [c.id, c.name]));
    ...
    categoryBreakdownData.forEach((item: any) => {
      ...
      const categoryName = catId ? (categoryMap.get(catId) || 'Other') : 'Other';
