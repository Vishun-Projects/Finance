...
export default function IncomeManagement() {
  ...
  const [statementMetadata, setStatementMetadata] = useState<any>(null); // Store metadata from parsing
  ...
  const fetchIncomes = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/income?userId=${user.id}&start=${startDate}&end=${endDate}&pageSize=10000&includeTotal=true`);
      console.log('🔄 INCOME COMPONENT - API Response status:', response.status);
      
      if (response.ok) {
        const responseData = await response.json();
        console.log('✅ INCOME COMPONENT - API Response data:', JSON.stringify(responseData, null, 2));
    ...
    try {
      ...
      if (isPdf) {
        ...
        console.log('🔍 FRONTEND: Received data:', {
          ...
          openingBalance: data.metadata?.openingBalance
        });
        ...
        console.log('🔍 FRONTEND: Metadata:', data.metadata);
        
        setParsedTransactions(transactionsToSet);
        setStatementMetadata(data.metadata || null); // Store metadata for later use
  ...
  const handleImportParsedTransactions = async () => {
    ...
    try {
      ...
      // Include metadata if available (for opening balance tracking)
      ...
      // Handle warnings and errors from balance validation
      ...
      if (result.balanceValidation) {
        console.log('💰 Balance validation:', {
          isValid: result.balanceValidation.isValid,
          isFirstImport: result.balanceValidation.isFirstImport,
          discrepancy: result.balanceValidation.discrepancy,
          warning: result.balanceValidation.warning,
          error: result.balanceValidation.error
        });
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      if (existingResponse.ok) {
        const existingResponseData = await existingResponse.json();
        // Handle paginated response structure
        const existingData = Array.isArray(existingResponseData) ? existingResponseData : (existingResponseData.data || []);
              ...
              <div>
                ...
                <Combobox
                  options={[
                    ...
                    { value: 'Auto Credit', label: 'Auto Credit' },
