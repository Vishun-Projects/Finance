...
function SettingsNavButton({
  ...
  return (
    ...
    >
      ...
      {isActive ? <span className="size-1.5 shrink-0 rounded-full bg-primary" aria-hidden /> : null}
    </button>
  );
}
...
export function SettingsDesktopLayout({
  ...
  const [advancedOpen, setAdvancedOpen] = useState(() =>
    ADVANCED_SECTION_IDS.includes(activeSection as SettingsSectionId),
  );

  useEffect(() => {
    if (ADVANCED_SECTION_IDS.includes(activeSection as SettingsSectionId)) {
      setAdvancedOpen(true);
    }
  }, [activeSection]);

  const primarySections = SETTINGS_SECTIONS.filter((s) => PRIMARY_SECTION_IDS.includes(s.id));
  const advancedSections = SETTINGS_SECTIONS.filter((s) => ADVANCED_SECTION_IDS.includes(s.id));
        ...
        <nav aria-label="Settings sections">
          ...
          <div className="mt-4">
            <button
              ...
              className="mb-2 flex w-full items-center justify-between px-1 text-xs font-medium uppercase tracking-wider text-hint"
