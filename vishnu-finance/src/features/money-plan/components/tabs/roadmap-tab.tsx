...
export function RoadmapTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="card-base px-3 py-3 sm:px-4">
      ...
      <div className="card-base overflow-hidden divide-y divide-border/60">
        {ROADMAP_MONTHS.map((month) => (
          <div key={month.label} className="px-3 py-3 sm:px-4">
                ...
                >
                  <div
                    className={cn(
                      ...
                      done[task.id] ? 'bg-foreground' : 'border-[1.5px] border-border/70 bg-transparent',
                    )}
                  >
                    {done[task.id] ? (
                      <svg
                  ...
                  <div className="flex-1">
                    <span
                      className={cn(
                        'text-[13px]',
                        done[task.id] ? 'text-hint line-through' : 'text-foreground',
                      )}
                    >
                      {task.text}
                    </span>
      ...
      <div className="card-base bg-surface/60 px-3 py-3 sm:px-4">
        <div className="mb-2.5 text-[13px] font-medium text-foreground">The One Rule</div>
        <div className="text-[13px] leading-relaxed text-muted-foreground">
          Har appraisal/increment ka <strong className="text-foreground">80% investments mein</strong>, 20%
          lifestyle.
        </div>
        <div className="mt-3 grid grid-cols-1 gap-1.5 border-t border-border/60 pt-3 sm:grid-cols-3">
          {(
            [
            ...
            <div key={age} className="rounded-lg bg-card/80 px-2 py-2.5 text-center">
