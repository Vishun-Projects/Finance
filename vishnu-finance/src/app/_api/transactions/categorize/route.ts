...
async function processCategorizationInBackground(
  ...
  for (let i = 0; i < transactionIds.length; i += batchSize) {
    ...
    try {
      ...
      const transactionsToCategorize = transactions.map((t: any) => {
      ...
      const updates = transactions.map(async (t: any, idx: number) => {
      ...
      try {
        const newlyCategorized = transactions.filter((t: any, idx: number) => {
          const result = results[idx];
