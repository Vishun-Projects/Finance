...
/**
 * Background categorization API
 * Processes transactions in batches and updates them asynchronously
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { userId, transactionIds, batchSize = 100 } = body as {
      userId: string;
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to start categorization job' },
      { status: 500 }
    );
  }
}
